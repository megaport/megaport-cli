...
func promptForBuyMVEDetails() (*megaport.BuyMVERequest, error) {
	// Initialize request
	req := &megaport.BuyMVERequest{}
	...
	vendorStr, err := prompt("Enter vendor (6wind, aruba, aviatrix, cisco, fortinet, palo_alto, prisma, versa, vmware, meraki) (required): ")
	if err != nil {
		return nil, err
	}
	if vendorStr == "" {
	...
	case "aviatrix":
		cloudInit, err := prompt("Enter cloud init data (required): ")
		if err != nil {
			return nil, err
		}
	...
	for {
		...
		description, err := prompt("Enter VNIC description: ")
		if err != nil {
			return nil, err
		}
		...
		vlanStr, err := prompt("Enter VLAN ID: ")
		if err != nil {
			return nil, err
		}
		vlan := 0
	...
	for {
		...
		value, err := prompt(fmt.Sprintf("Enter value for tag '%s': ", key))
		if err != nil {
			return nil, err
		}
...
func promptForUpdateMVEDetails(mveUID string) (*megaport.ModifyMVERequest, error) {
	...
	name, err := prompt("Enter new MVE name (leave empty to keep current): ")
	if err != nil {
		return nil, err
	}
	if name != "" {
	...
	costCentre, err := prompt("Enter new cost centre (leave empty to keep current): ")
	if err != nil {
		return nil, err
	}
	if costCentre != "" {
	...
	contractTermStr, err := prompt("Enter new contract term (1, 12, 24, or 36 months, leave empty to keep current): ")
	if err != nil {
		return nil, err
	}
	if contractTermStr != "" {

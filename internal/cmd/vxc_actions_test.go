	...
	tests := []struct {
	...
	}{
		{
			...
			setupMock: func(t *testing.T, m *mockVXCService) {
				...
				buildUpdateVXCRequestFromPrompt = func(vxcID string) (*megaport.UpdateVXCRequest, error) {
					name := "New VXC Name"
					rateLimit := 200
					term := 24
					costCentre := "New Cost"
		...
		},
		{
			...
			setupMock: func(t *testing.T, m *mockVXCService) {
				...
				buildUpdateVXCRequestFromPrompt = func(_ string) (*megaport.UpdateVXCRequest, error) {
					return nil, fmt.Errorf("no update parameters provided")
				}

...
### Fixed
- `mve buy` flags now reject malformed vNIC entries (non-object entries, non-numeric VLANs) with a clear error instead of silently dropping or zeroing them, matching JSON input. Interactive `mve buy` now normalizes vendor and product-size case (e.g. `Cisco`, `medium`), matching flags and JSON, instead of failing at the end of the prompt sequence (ESD-1589)
